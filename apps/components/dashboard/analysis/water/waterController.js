LEEDOnApp.controller('waterAnalysisController', function($rootScope, $scope) {
    ...
    $('.analysis_performance_energy_template_img').attr('src', 'assets/images/water_gray.svg');
    $('.analysis_dynamic_section_value').attr('src', 'assets/images/water.svg');
    $('#analysisincrease_by_img').attr('src', 'assets/images/water_white.png');                

    $.ajax({
            url: 'assets/json/waterinfo_' + $scope.leed_id + '.json',
            type: "GET",
    }).done(function(data) {

            window.getAnalysisData.waterAnalysis_data = data;
            if (data){
            	setTimeout(function(){
                    $scope.initialize_slider("water");
                }, 1000);
            }
